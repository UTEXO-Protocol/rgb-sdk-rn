...
function detectPsbtType(psbtBase64: string): 'create_utxo' | 'send' {
  ...
  try {
    ...
    if (hex.includes(testnetHex) || hex.includes(mainnetHex)) {
      const testnetWithApostrophe = Buffer.from(rgbTestnetStr + "'").toString(
        'hex'
      );
      const mainnetWithApostrophe = Buffer.from(rgbMainnetStr + "'").toString(
        'hex'
      );
      ...
      if (
        hex.includes(testnetWithApostrophe) ||
        hex.includes(mainnetWithApostrophe) ||
        hex.includes(testnetWithSlash) ||
        hex.includes(mainnetWithSlash)
      ) {
        return 'send';
      }
    }
...
function createCustomCoinTypeDescriptors(
  ...
  const tempDesc = Descriptor.newBip86(
    secretKey,
    KeychainKind.External,
    bdkNetwork
  );
  ...
  const { origin: accountOrigin, key: accountKeyOnly } =
    extractKeyAndOrigin(accountKeyString);

  const externalOrigin =
    accountOrigin || `[${originFingerprint}/86'/${customCoinType}'/0']`;
...
export async function signPsbt(
  ...
  try {
    ...
    if (psbtType === 'create_utxo') {
      const descriptors = createStandardDescriptors(
        mnemonicObj,
        normalizedNetwork
      );
      ...
      const descriptors = createCustomCoinTypeDescriptors(
        mnemonicObj,
        normalizedNetwork
      );
      externalDescriptor = descriptors.external;
    ...
    if (!isSigned && psbtType === 'create_utxo') {
      logger.debug(
        'Signing with create_utxo descriptors failed, trying send descriptors'
      );
      const descriptors = createCustomCoinTypeDescriptors(
        mnemonicObj,
        normalizedNetwork
      ...
      logger.debug(
        'Signing with send descriptors failed, trying create_utxo descriptors'
      );
      const descriptors = createStandardDescriptors(
        mnemonicObj,
        normalizedNetwork
    ...
    logger.debug(
      'External Descriptor (with secret):',
      externalDescriptorString
    );
    logger.debug(
      'Internal Descriptor (with secret):',
    ...
    throw new CryptoError(
      `Unexpected error during PSBT signing: ${errorMessage}`,
      error as Error
    );
  }
}
...
export async function signMessage(params: SignMessageParams): Promise<string> {
  ...
  const signature = Buffer.from(signSchnorr(messageHash, privateKey)).toString(
    'base64'
  );
  return signature;
}
...
export async function estimatePsbt(
  ...
  throw new CryptoError(
    'PSBT estimation not yet implemented. Use BDK or another library for fee estimation.'
  );
}
